...
Under the formants: a sawtooth with a wide pitch glide (up hard into the middle, falling away), a few Hz of frequency jitter because animals are not in tune with themselves, breath noise through the same vocal tract, a low-passed chest layer so the bandpasses do not sound nasal, and amplitude roughness at vocal-fold rate. No assets, no network, no CSP allowance.
...
**It is a convincing creature, not an impression.** Chewbacca's voice is Ben Burtt's recordings of bears, walruses and camels; synthesis gets you the register, the formant movement and the roughness, and stops there. Actually sounding like him would mean shipping Star Wars audio, which is not this repo's to distribute. The synth is tuned by measurement rather than vibes — rendered offline, the spectrum peaks at 450 Hz with formant structure at 730 and 1090 Hz, carrying 2.5× the energy of the sub-bass, which is what "vocal rather than rumble" means numerically. The Vocoder panel hides itself for him, because there is no device voice, pitch or rate to tune.
