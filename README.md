...
**And he is not spoken — he is synthesised.** No text-to-speech voice can say "Rrrwwwgg": every voice a device ships is a human-language voice, so it pronounces the growl as an English speaker reading nonsense syllables. Pitch and rate cannot fix that; the phonetics are wrong, not the tuning. So Chewbacca bypasses `speechSynthesis` entirely and `roar.js` builds the sound with Web Audio. The part that matters is the **formant bank**: three swept bandpass filters that move through vowel shapes (`er → aa → oo`) over each growl. A Wookiee moan is a mouth opening and closing while a pitch glides underneath it, and that sweep is the "rrraaa-ooowww". A single lowpass cannot make it, however the fundamental is tuned — the first attempt at this had one, and sounded like a bear in a barrel.

Under the formants: **subharmonics** at roughly f0/3 and f0/2, which is the actual anatomy of a growl — rough vocal folds period-double, and that is both what makes a big animal sound big and where the bass comes from. Plus a sawtooth with a wide pitch glide, a few Hz of frequency jitter because animals are not in tune with themselves, breath noise through the same vocal tract, and a low-passed chest layer so the bandpasses do not sound nasal.

**The roughness must not be periodic.** An earlier version modulated amplitude with a clean sine at 26-42 Hz, and that is a motorbike: 20-50 Hz of regular pulsing is an engine firing rate and the ear names it instantly. The modulation is now low-passed noise through two cascaded stages — one was not steep enough and left a 17 Hz bump, which is squarely bike-idle territory. No assets, no network, no CSP allowance.

The text shapes the roar without being pronounced: each token becomes one growl whose length follows the token's, capitals and exclamation marks raise the "heat" (pitch travel, rasp, tremolo depth), and the whole line is seeded from the text so a given growl always has the same shape. Long roars in the transcript sound long; angry ones sound angry. That is the only way meaning survives when the words are not words.

### Using a real recording instead

`web/audio/` is a hook with nothing in it. Drop audio files there, list them in `roars.json`, and Chewbacca plays a clip instead of synthesising — chosen deterministically from the text, with a slight playback-rate shift so repeats do not sound looped. No manifest, which is the default, and the synth answers. A file that will not decode falls back to synthesis rather than leaving him mute.

**The repo ships no audio and will not.** The obvious recordings are Lucasfilm's and this project has no right to redistribute them; anything committed to `web/audio/` is served publicly by Pages and Render. See `web/audio/README.md` for where freely-licensed material can be found — bear, walrus and camel recordings were Ben Burtt's original ingredients, and layering a few gets closer to his method than any synthesiser.

**It is a convincing creature, not an impression.** Chewbacca's voice is Ben Burtt's recordings of bears, walruses and camels; synthesis gets you the register, the formant movement and the roughness, and stops there. Actually sounding like him would mean shipping Star Wars audio, which is not this repo's to distribute. The synth is tuned by measurement rather than vibes. Rendered offline: peak 0.94 with no clipping, bass (40-150 Hz) carrying ~3.5× the formant energy so it is bass-forward while the vowels still read, and — the one that matters — an amplitude-envelope analysis scoring **1.8× the average**, against **27×** for a reference saw-plus-30 Hz-AM signal built to sound like a motor. That calibration is the test: chasing an absolute threshold was meaningless until there was something known-bad to compare against. The Vocoder panel hides itself for him, because there is no device voice, pitch or rate to tune.
...
web/                      ← the whole application
  ...
  js/voice.js             ← speechSynthesis + SpeechRecognition, and the routing
  js/roar.js              ← Web Audio Shyriiwook, for the one who is not spoken
